...
exports.main = async (event, context) => {
  ...
  if (!openid) {
    console.error('[ReviewReject] 无法获取openid');
    return errorResponse(7001, '无法获取用户信息');
  }

  const { review_id, reject_reason } = event;
  ...
  if (!reject_reason || reject_reason.trim() === '') {
    return errorResponse(7010, '请填写拒绝原因');
  }

  console.log('[ReviewReject] 审核拒绝, openid:', openid.substring(0, 8) + '...', 'review_id:', review_id);
  ...
  try {
    ...
    try {
      ...
      console.log('[ReviewReject] 审核拒绝成功');
    ...
    console.error('[ReviewReject] 审核失败:', e);
    return errorResponse(7011, '审核操作失败');
  }
};
